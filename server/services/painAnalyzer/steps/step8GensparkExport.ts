...
`);
  ...
  // ── Slide 2-6: 概念説明 (博報堂・SHIBUYA109 lab. 準拠) ──
  md.push(`## Slide 2: 【概念1】マーケティングの変革
- **内容**: デモグラ（属性）から界隈（熱量）へのシフト。
- **キーメッセージ**: 市場はカテゴリ起点ではなく、「好き」や興味関心でつながる生活者の集まりとして見直すべき。「大衆の時代」「分衆の時代」を経て、現在は「個の時代」に入り、「好き」を軸にした小さな集まりが消費の起点になった。
- **データ**: 「界隈」認知率 全体45.8%、女性10代 80.6%。自分を界隈に属すると認識 全体28.1%、女性10代 61.2%。界隈情報をもとに購買した経験 全体22.5%、女性10代 56.3%。(Source: 博報堂・SHIBUYA109 lab.)
- **図解**: ピラミッド型(属性で括る)から、ポップな人物アイコンが集まる「界隈（バブル）」への変化図。
`);

  md.push(`## Slide 3: 【概念2】界隈の構造
- **内容**: 境界線がなく重なり合い、人が回遊する構造。
- **キーメッセージ**: 界隈はファンコミュニティとは異なり「ゆるさ」と「距離感の自己調整」が本質。一人が複数の界隈に所属し回遊する。厳密なセグメントではなく重なりを前提に設計する。
- **界隈の7分類**: 情報交換系 / 趣味系 / 推し活・オタ活系 / 世界観系 / 連帯系 / 「あるある」系 / ネタ系
- **図解**: 重なり合うオレンジ・ベージュの円と、その間を行き来する人物イラスト。大界隈の中に中小界隈が入れ子状に存在する構造。
`);

  md.push(`## Slide 4: 【概念3】消費のメカニズム — 界隈内消費と界隈伝播消費
- **内容**: 「熱量×共感」による2段階の消費伝播。
- **界隈内消費**: 界隈の内部で評判が回り、多数に購入が広がる。成立条件＝界隈内部の熱量、共感、個別具体的ニーズが強いこと。
- **界隈伝播消費**: ある界隈で流行したものが別の界隈にも波及する。成立条件＝受け手の界隈から見て活用可能で、元の界隈に信頼や好意があること。
- **戦略**: Core Layer (A-C) で界隈内消費を起こし、Expansion Layer (D-E) で伝播消費を狙う。
- **図解**: 中心からオレンジ色の波紋が広がり、隣接する界隈バブルに到達する様子。
`);

  md.push(`## Slide 5: 【概念4】SEESASモデルと加速メカニズム
- **内容**: 界隈起点の消費行動モデル「SEESAS」。
- **図解**: 以下の6段階がスパイラル状（渦）に回転しながら拡大していくインフォグラフィックス。
  1. **Sympathy**（共感）— 「いいな」と感じる
  2. **Enthusiasm**（熱狂）— 「これ最高」と没入する
  3. **Expression**（発信）— 「みんなにも教えたい」と投稿する
  4. **Spread**（拡散）— 界隈内で広がる
  5. **Action**（行動）— 購入・体験する
  6. **Sustainability**（定着）— 継続・リピートする
- **加速ロジック**: 「界隈内の熱量が高まるほど回転速度が上がり、遠心力で他界隈へ飛び火する」
- **施策対応**: 右脳アプローチ＝S/Eを加速、左脳アプローチ＝A/Sを加速
`);

  md.push(`## Slide 6: 【概念5】企業参入の4原則
- **内容**: 界隈にアプローチする際の絶対ルール。
  1. **カテゴライズしない**: 「〇〇界隈向け」ではなく「〇〇好きの人へ」。勝手にラベル化しない。
  2. **リスペクトを忘れない**: 界隈の価値観や姿勢への敬意が伝わる設計。表面的コラボより理解の深さ。
  3. **ポジティブ文脈で発信**: 優越感ではなく「一緒に楽しめる」表現。利他的共有が広がりを生む。
  4. **活用シーンを提案**: 商品説明ではなく「界隈の活動の中でどう使うと嬉しいか」の解像度を上げる。
- **プロセス**: 見つける → 学ぶ → 盛り上げる → 広がる
