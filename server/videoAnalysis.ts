...
function safeJsonParse(text: string): any {
  try {
    ...
    // Fix bad Unicode escapes (e.g. \uXXXX where XXXX is invalid)
...
【動画情報】
説明文: ${[...input.description].slice(0, 500).join("")}
...
export async function analyzeSentimentAndKeywordsBatch(
  ...
  const videoList = inputs.map((v, i) =>
    `[動画${i}]\n説明: ${[...v.description].slice(0, 300).join("")}\nハッシュタグ: ${v.hashtags.join(", ")}\nOCRテキスト: ${[...v.ocrTexts.filter(t => t).join(" ")].slice(0, 100).join("")}`
  ).join("\n\n");
...
export async function generateAnalysisReport(jobId: number): Promise<void> {
  ...
  const STOP_WORDS = new Set([
    ...
    "her", "our", "their", "what", "which", "who", "whom", "how", "where",
    "when", "why", "all", "each", "every", "any", "some", "such",
    "than", "too", "very", "just", "about", "up", "out", "if", "then",
    "also", "only", "more", "most", "other", "over", "here", "there",
    // 日本語助詞・一般的すぎる語
    "する", "いる", "ある", "なる", "れる", "られる", "せる", "させる",
    "こと", "もの", "ため", "ところ", "よう", "ほう", "くらい",
    "Japan", "japan", "Japanese", "TikTok", "tiktok",
  ]);

  const isValidWord = (w: string): boolean => {
    // 空文字・短すぎる
    if (w.length < 2) return false;
    // 長すぎるフレーズ（8文字超は文章の可能性が高い）
    if (w.length > 12) return false;
    // ストップワード
    if (STOP_WORDS.has(w) || STOP_WORDS.has(w.toLowerCase())) return false;
    // 数字・日付のみ
    if (/^[\d\s\/\-〜~年月日:：.]+$/.test(w)) return false;
    // 英語の短い一般語（3文字以下の英字のみ）
    if (/^[a-zA-Z]{1,3}$/.test(w)) return false;
    // 記号だけ
    if (/^[【】「」『』（）()#@\s]+$/.test(w)) return false;
    // 先頭が【や「で始まる長い修飾付きフレーズ
    if (/^[【「『]/.test(w) && w.length > 8) return false;
    return true;
  };
  ...
  const buildWordCountMap = (words: string[], ocrAudioWords: string[]): Map<string, number> => {
    ...
    for (const w of words) {
      if (isValidWord(w)) counts.set(w, (counts.get(w) || 0) + 1);
    }
    return counts;
  };
...
- analysis: 分析詳細（150-300文字）。固有名詞、具体的な引用コメント、および「なぜその反応が起きたのか」の市場背景・心理的因果関係を含めること。動画を参照する際は「動画[ID]」ではなく「@アカウント名」の形式で記載すること。
...
${videosData.slice(0, 20).map(v => {
  ...
  return `- @${v.accountName} [${v.videoId}]: ${[...(v.description || "")].slice(0, 80).join("")} (${v.sentiment}, ${vw.toLocaleString()}再生, ER:${er}%)`;
}).join("\n")}`,
        },
      ],
      response_format: {
