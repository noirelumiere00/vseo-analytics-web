...
- `--max`（既定 30）= 取得する上位件数。
- **`--keyword`（`--kw`）= ハッシュタグではなく「キーワード検索」（IGアプリ版の検索＝`instagram.com/popular/<語>`）の表示順位を取得**。例: `INSTAGRAM_SESSION_ID=… npx tsx scripts/scrape-ig-ranking.ts "N高" "N高等学校" --keyword --max 30 --own-reels data/ig-own-reels.txt`。`#` ではなく**素のキーワード**を渡す。中身は IG の `fbsearch/web/top_serp` API を **Node fetch で直接**取得する（Chromium 不要）ので、**このクラウド（コンテナ）からも安定して回る**（`sessionid` ＋ `ds_user_id` Cookie を内部で付与）。`next_max_id` で 30 件までページ送り。出力は `out/ig-keyword-<語>-<日時>.{html,csv,json}`。`--own/--own-reels/--reels-only/--proposal` と併用可。**keyword はセッション必須**（Apify フォールバック無し）。
