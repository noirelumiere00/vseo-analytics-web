...
export function ReportSection({
  ...
  return (
    ...
    >
      ...
      <div
        style={{
          background: "linear-gradient(135deg, #0f2439 0%, #1a3a5c 50%, #1e4d7a 100%)",
      ...
      >
        ...
        <div style={{ maxWidth: 720, margin: "0 auto", position: "relative", zIndex: 1 }}>
          ...
          <div style={{ display: "flex", alignItems: "center", gap: 8, flexWrap: "wrap" }}>
            ...
            <span style={{ fontSize: 13, color: "rgba(255,255,255,0.5)" }}>{date}</span>
          </div>
          ...
          >
            {[
              { label: "分析動画数", value: `${videoCount}本` },
              { label: "プラットフォーム", value: platform },
              { label: "分析手法", value: "LLM側面抽出" },
            ].map((m) => (
              <div
                key={m.label}
                style={{
      ...
      <div style={{ padding: "0 24px" }}>
        ...
        <div style={{ display: "flex", gap: 12, marginBottom: 24, flexWrap: "wrap" }}>
          <StatCard
            label="最高評価"
            value={`${Math.max(...aspects.map((a) => a.pos))}%`}
            sub={aspects.find((a) => a.pos === Math.max(...aspects.map((a) => a.pos)))?.name}
            color="#0d9255"
          />
          <StatCard
            label="平均ポジティブ率"
            value={`${avgPositive}%`}
            sub={`全${aspects.length}側面`}
            color="#2e75b6"
          />
          <StatCard
            label="要改善"
            value={`${Math.max(...aspects.map((a) => a.neg))}%`}
            sub={aspects.find((a) => a.neg === Math.max(...aspects.map((a) => a.neg)))?.name}
            color="#c0392b"
          />
        </div>
